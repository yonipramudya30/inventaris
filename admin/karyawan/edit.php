...
$stmt = $conn->prepare("SELECT id, username, nama FROM users WHERE id = ? AND role = 'karyawan'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        if (!empty($password)) {
            ...
            $stmt = $conn->prepare("UPDATE users SET username = ?, nama = ?, password = ? WHERE id = ?");
            $stmt->bind_param('sssi', $username, $nama, $hashed_password, $id);
        } else {
            // Update tanpa mengubah password
            $stmt = $conn->prepare("UPDATE users SET username = ?, nama = ? WHERE id = ?");
...
<head>
    ...
    <title>Edit Karyawan - Sistem Inventaris</title>
